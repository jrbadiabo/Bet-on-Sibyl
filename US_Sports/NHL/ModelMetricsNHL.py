...
class ModelMetricsNHL(object):
    ...
    def get_metrics(self, tableau_input_filename, matchup_output_us, cs_team_stats_filename):
        ...
        sibyl_metrics_df.to_csv('Sibyl_metrics_NHL.csv', mode='w+', index=True)
        month_teams_df.to_csv('Month_teams_NHL.csv', mode='w+', index=True)
        today_games_divstrat_df.to_csv('today_games_divstrat_NHL.csv', mode='w+', index=True)
