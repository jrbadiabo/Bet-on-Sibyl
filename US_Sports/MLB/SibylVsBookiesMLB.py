...
class AcquireSibylVsBookiesMLB(object):
    ...
    def __call__(self):
        if self.season_over == 'No':
            ...
            self.sibyl_vs_bookies_simulation(self.mlb_us_p_df)
            print "OK"
    ...
    def fix_str(self):
        ...
        if ':' in self:
            if 'pen.' in self:
                self = self[:self.find('pen.')].strip()  # Used for baseball model
            self = self.split(':')
    ...
    def get_team_names_list(self):
        ...
        url = "http://www.oddsportal.com/baseball/usa/mlb/outrights/"
    ...
    def clean_data(self, game_df, odd_tm_list, cs_team_stats_filename, tableau_input_filename):
        ...
        db_tm_list = sorted(current_sea_df['Tm'].tolist())
        ...
        mlb_eu_p_df['True_Result'] = mlb_us_p_df.apply(
            lambda x: x['Home_Team'] if x['True_Result'] == 1 else x['Visitor_Team'], axis=1)

        mlb_us_p_df = mlb_us_p_df.reset_index(drop=True)
        mlb_eu_p_df = mlb_eu_p_df.reset_index(drop=True)

        mlb_us_p_df.to_csv('Sibyl_vs_Bookies_US_MLB.csv', mode='w+', index=True, index_label='ID')

        mlb_eu_p_df.to_csv('Sibyl_vs_Bookies_EU_MLB.csv', mode='w+', index=True, index_label='ID')

        self.mlb_us_p_df = mlb_us_p_df
        self.mlb_eu_p_df = mlb_eu_p_df
    ...
    def sibyl_vs_bookies_simulation(self, df):
        ...
        plt.savefig('Sibyl_vs_Bookies_MLB.png', format='png')
        plt.close(fig)
        ...
        metrics_df.to_csv('Sibyl_vs_Bookies_metrics_MLB.csv', mode='w+', index=True, index_label='ID')


if __name__ == '__main__':
    x = AcquireSibylVsBookiesMLB("http://www.oddsportal.com/baseball/usa/mlb/results/", \
                                 "http://www.oddsportal.com/baseball/usa/mlb/results/#/page/{}/", \
                                 "mlb_team_stats_2016_2016.csv", \
                                 "mlb_tableau_output_2016.csv")
