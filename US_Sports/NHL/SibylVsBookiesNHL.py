...
class AcquireSibylVsBookiesNHL(object):
    ...
    def __call__(self):
        if self.season_over == 'No':
            ...
            self.sibyl_vs_bookies_simulation(self.nhl_us_p_df)
            print "OK"
    ...
    def fix_str(self):
        ...
        if ':' in self:
            if 'pen.' in self:
                self = self[:self.find('pen.')].strip()  # Used for hockey model
            self = self.split(':')
    ...
    def get_team_names_list(self):
        ...
        url = "http://www.oddsportal.com/hockey/usa/nhl/outrights/"
    ...
    def clean_data(self, game_df, odd_tm_list, cs_team_stats_filename, tableau_input_filename):
        ...
        nhl_eu_p_df['True_Result'] = nhl_us_p_df.apply(
            lambda x: x['Home_Team'] if x['True_Result'] == 1 else x['Visitor_Team'], axis=1)

        nhl_us_p_df = nhl_us_p_df.reset_index(drop=True)
        nhl_eu_p_df = nhl_eu_p_df.reset_index(drop=True)

        self.nhl_us_p_df = nhl_us_p_df
        self.nhl_eu_p_df = nhl_eu_p_df

        nhl_us_p_df.to_csv('Sibyl_vs_Bookies_US_NHL.csv', mode='w+', index=True, index_label='ID')

        nhl_eu_p_df.to_csv('Sibyl_vs_Bookies_EU_NHL.csv', mode='w+', index=True, index_label='ID')
    ...
    def sibyl_vs_bookies_simulation(self, df):
        ...
        plt.savefig('Sibyl_vs_Bookies_NHL.png', format='png')
        plt.close(fig)
        ...
        metrics_df.to_csv('Sibyl_vs_Bookies_metrics_NHL.csv', mode='w+', index=True, index_label='ID')


if __name__ == '__main__':
    x = AcquireSibylVsBookiesNHL(
        "No", \
