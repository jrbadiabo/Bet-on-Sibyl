...
class NHLMakePredictions(object):
    # Apply ML techniques to the NHL scrapped data and use of the resulting model to make predictions on unplayed games
    # NB: unplayed games are in the nhl_db_name_FOR_THE_CURRENT_SEASON (ex 'nhl_db_name_2017')
    # => season on which you want to
    # make predictions

    from ScrapeNHLTeamStats import AcquireNHLTeamStats
    from ScrapeNHLGameStats import AcquireNHLGameStats
    from PrepareForMLTechNHL import PrepareForML

    def __init__(self, current_season, ps_feature_filename, nhl_db_name):
        self.data = np.load(ps_feature_filename)
        self.current_season = current_season
        self.tableau_output_filename = "nhl_tableau_output_" + str(current_season) + '.csv'
        self.X = self.data['X']
        self.y = self.data['y']
        self.nhl_db_name = nhl_db_name

    def __call__(self):
        print "Scraping NHL current season data for update...\n"
        self.acquire_current_season_data(self.current_season)
        # Train once again our data from 'nhl_features.npz' so as to show the learning curves
    ...
    def acquire_current_season_data(self, current_season):
        ...
        team_data_filename = 'nhl_team_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        game_data_filename = 'nhl_game_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        datetime_filename = 'date_nhl_game_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        nhl_db_filename = 'nhl_team_data_' + str(current_season) + '.db'
        cs_feature_filename = 'nhl_features_' + str(current_season) + '.npz'
        # If you want to filter,
        # uncomment and make changes to the corresponding section in the 'AcquireGameStats' class

        # Scrape nhl data for the current season
        print "Scraping Team Stats..."
        nhl_team_data = self.AcquireNHLTeamStats(current_season, current_season)
        nhl_team_data()
        print "OK\n"

        print "Scraping Game Stats..."
        nhl_game_data = self.AcquireNHLGameStats(current_season, current_season)
        nhl_game_data()
        print "OK\n"

        print "Preprocessing updated data for Machine Learning..."
        # Prepare for ML predictions 
        pml = self.PrepareForML(game_data_filename, nhl_db_filename, team_data_filename)
    ...
    # 'instantiating + training to the data' and only 'instantiating'
    ...
    def make_predictions(self, team1, team2, scale_data=False):
        ...
        con = lite.connect(self.nhl_db_name)
        with con:
            cur = con.cursor()
