...
class NFLMakePredictions(object):
    # Apply ML techniques to the NFL scrapped data and use of the resulting model to make predictions on unplayed games
    # NB: unplayed games are in the nfl_db_name_FOR_THE_CURRENT_SEASON (ex 'nfl_db_name_2016')
    # => season on which you want to
    # make predictions

    from ScrapeNFLTeamStats import AcquireNFLTeamStats
    from ScrapeNFLGameStats import AcquireNFLGameStats
    from PrepareForMLTechNFL import PrepareForML

    def __init__(self, current_season, ps_feature_filename, nfl_db_name):
        self.data = np.load(ps_feature_filename)
        self.current_season = current_season
        self.tableau_output_filename = "nfl_tableau_output_" + str(current_season) + '.csv'
        self.X = self.data['X']
        self.y = self.data['y']
        self.nfl_db_name = nfl_db_name

    def __call__(self):
        print "Scraping NFL current season data for update...\n"
        self.acquire_current_season_data(self.current_season)
        # Train once again our data from 'nfl_features.npz' so as to show the learning curves
    ...
    def acquire_current_season_data(self, current_season):
        ...
        team_data_filename = 'nfl_team_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        game_data_filename = 'nfl_game_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        datetime_filename = 'date_nfl_game_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        nfl_db_filename = 'nfl_team_data_' + str(current_season) + '.db'
        cs_feature_filename = 'nfl_features_' + str(current_season) + '.npz'
        # If you want to filter,
        # uncomment and make changes to the corresponding section in the 'AcquireGameStats' class

        # Scrape nfl data for the current season
        print "Scraping Team Stats..."
        nfl_team_data = self.AcquireNFLTeamStats(current_season, current_season)
        nfl_team_data()
        print "OK\n"

        print "Scraping Game Stats..."
        nfl_game_data = self.AcquireNFLGameStats(current_season, current_season)
        nfl_game_data()
        print "OK\n"

        print "Preprocessing updated data for Machine Learning..."
        # Prepare for ML predictions 
        pml = self.PrepareForML(game_data_filename, nfl_db_filename, team_data_filename)
