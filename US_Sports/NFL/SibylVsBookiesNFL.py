...
class AcquireSibylVsBookiesNFL(object):
    ...
    def __call__(self):
        if self.season_over == 'No':
            ...
            self.sibyl_vs_bookies_simulation(self.nfl_us_p_df)
            print "OK"
    ...
    def fix_str(self):
        ...
        if ':' in self:
            if 'pen.' in self:
                self = self[:self.find('pen.')].strip()  # Used for american-football model
            self = self.split(':')
    ...
    def get_team_names_list(self):
        ...
        url = "http://www.oddsportal.com/american-football/usa/nfl/outrights/"
    ...
    def clean_data(self, game_df, odd_tm_list, cs_team_stats_filename, tableau_input_filename):
        ...
        nfl_us_p_df['True_Result'] = nfl_us_p_df.apply(
        ...
        nfl_eu_p_df['True_Result'] = nfl_us_p_df.apply(
            lambda x: x['Home_Team'] if x['True_Result'] == 1 else x['Visitor_Team'], axis=1)
        ...
        nfl_us_p_df.to_csv('Sibyl_vs_Bookies_US_NFL.csv', mode='w+', index=True, index_label='ID')

        nfl_eu_p_df.to_csv('Sibyl_vs_Bookies_EU_NFL.csv', mode='w+', index=True, index_label='ID')
    ...
    def sibyl_vs_bookies_simulation(self, df):
        ...
        plt.savefig('Sibyl_vs_Bookies_NFL.png', format='png')
        plt.close(fig)
        ...
        metrics_df.to_csv('Sibyl_vs_Bookies_metrics_NFL.csv', mode='w+', index=True, index_label='ID')

        # ex of use
        # x = AcquireSibylVsBookiesNFL("http://www.oddsportal.com/american-football/usa/nfl/results/",\
