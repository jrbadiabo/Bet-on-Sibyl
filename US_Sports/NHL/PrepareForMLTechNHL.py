...
class PrepareForML(object):
    ...
    def __init__(self, scoring_filename, nhl_db_name, team_stats_csv_filename):
    ...
    def __call__(self, features_filename):
        # Looping through the csv file 'nhl_game_stats_year1_year2.csv" and creates a feature vector
        # for each game played. The results are stored in an array 'nhl_features.npz' in the current directory
    ...
    def process_game(game, cursor):
        ...
        # These elements refer to match.
        # This function queries the SQL database 'nhl_db_name' created via the 'process_raw_data'
    ...
    def process_raw_data(self, team_stats_csv_filename, what_to_do='sql'):
        ...
        # then a sqlite table named 'Team_Stats' is created in the database titled 'nhl_db_name'

        df = read_csv(team_stats_csv_filename)
        features = ['W', 'L', 'OL', 'PTS_Perc', 'GF', 'GA', 'SRS', 'SOS', 'TG_Per_G', 'PP', 'PPO', 'PP_Perc', 'PPA',
                    'PPOA', 'PK_Perc', 'SH', 'SHA', 'S', 'S_Perc', 'SA', 'SV_Perc']
