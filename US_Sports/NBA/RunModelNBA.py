...
def plot_confusion_matrix(cm, title='Confusion matrix'):
    ...
    # ex of use : x = NFLMakePredictions(2016, "nfl_features_2000_2015.npz", "nfl_team_data_2016.db")
...
class NBAMakePredictions(object):
    # Apply ML techniques to the NBA scrapped data and use of the resulting model to make predictions on unplayed games
    # NB: unplayed games are in the nba_db_name
    # FOR_THE_CURRENT_SEASON (ex 'nba_db_name_2016') => season on which you want to
    # make predictions

    from ScrapeNBATeamStats import AcquireNBATeamStats
    from ScrapeNBAGameStats import AcquireNBAGameStats
    from PrepareForMLTechNBA import PrepareForML

    def __init__(self, current_season, ps_feature_filename, nba_db_name):
        self.data = np.load(ps_feature_filename)
        self.current_season = current_season
        self.tableau_output_filename = "nba_tableau_output_" + str(current_season) + '.csv'
        self.X = self.data['X']
        self.y = self.data['y']
    ...
    def __call__(self):
        print "Scraping NBA current season data for update...\n"
        self.acquire_current_season_data(self.current_season)
        # Do the scraping and preprocessing jobs to have the
        # 'nba_features_CURRENT_SEASON.npz' so as to make predictions by then
    ...
    def acquire_current_season_data(self, current_season):
        ...
        team_data_filename = 'nba_team_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        game_data_filename = 'nba_game_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        datetime_filename = 'date_nba_game_stats_' + str(current_season) + '_' + str(current_season) + '.csv'
        nba_db_filename = 'nba_team_data_' + str(current_season) + '.db'
        cs_feature_filename = 'nba_features_' + str(current_season) + '.npz'
        # If you want to filter,
        # uncomment and make changes to the corresponding section in the 'AcquireGameStats' class

        # Scrape nba data for the current season
        print "Scraping Team Stats..."
        nba_team_data = self.AcquireNBATeamStats(current_season, current_season)
        nba_team_data()
        print "OK\n"

        print "Scraping Game Stats..."
        nba_game_data = self.AcquireNBAGameStats(current_season, current_season)
        nba_game_data()
        print "OK\n"

        print "Preprocessing updated data for Machine Learning..."
        # Prepare for ML predictions 
        pml = self.PrepareForML(game_data_filename, nba_db_filename, team_data_filename)
    ...
    def make_tableau_file(self, game_data_filename, datetime_filename):
        # Produces a csv file
        # containing predicted and actual game results for the current season that can see in Tableau
    ...
    # 'intanciating + training to the data' and only 'instanciating'
    ...
    def data_size_response(self, model, score_func, prob=True, n_subsets=10):
        ...
        # defining a var that take the value of n_subsets and creates nstart,
        # tosp_subsets with correspondin gsize. "linspace returns num
        # evenly spaced samples, calculated over the interval [].
        #  shape[0] of an array y of dimension (n,p) merely means
        # 'n', => n of rows
